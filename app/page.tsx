...
export default function HomePage() {
  ...
  const submitLead = () => setSubmitted(true);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
      ...
      <section id="features" className="max-w-6xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-3 gap-5">
          <Feature
            ...
            desc="RFA scoring, segments, next best action — refreshed nightly."
          />
          <Feature
            icon={<Users />}
            title="Personalized Outreach"
            desc="3-touch sequences, thank-yous, and call scripts tailored to each donor."
          />
          <Feature
            icon={<Zap />}
            title="Grant Co-Writer"
            desc="Turn program notes into tailored need statements, budgets, and outcomes."
          />
        </div>
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-4 py-10">
        <div className="flex items-baseline justify-between">
          ...
          <Link href="/pricing" className="text-sm underline hover:no-underline">
            See details →
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <p className="text-slate-600 mt-2">
              Tell us about your organization. We’ll set up a 30-minute session and a customized demo.
            </p>
            ...
            <div className="mt-6 space-y-4">
              ...
              <button
                onClick={submitLead}
                className="rounded-2xl bg-slate-900 text-white px-4 py-2 flex items-center"
              >
  ...
  return (
    <div className={`rounded-2xl border ${highlighted ? "ring-2 ring-slate-900" : ""} bg-white`}>
      <div className="p-6">
        ...
        <Link
          href="/pricing"
          className="mt-5 w-full inline-block text-center rounded-2xl bg-slate-900 text-white px-4 py-2"
        >
          Choose plan
        </Link>
      </div>
    ...
    <div>
      ...
      <div className="h-48 mt-2">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={netData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
            <defs>
            ...
            <YAxis />
            <Tooltip />
            <Area type="monotone" dataKey="actual" stroke="#0f172a" fill="url(#a)" />
            <Area type="monotone" dataKey="scenario" stroke="#64748b" fill="url(#b)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-3 mt-4">
        <div className="bg-slate-50 rounded-xl p-3 border">
          ...
          <div className="text-emerald-600 text-xs">+8% vs last month</div>
        </div>
        <div className="bg-slate-50 rounded-xl p-3 border">
          <div className="text-xs text-slate-500">Projected 90-day</div>
          <div className="text-2xl font-semibold">$256,900</div>
          <div className="text-emerald-600 text-xs">+12% with scenario</div>
        </div>
        <div className="bg-slate-50 rounded-xl p-3 border">
          <div className="text-xs text-slate-500">Open actions</div>
          <div className="text-2xl font-semibold">37</div>
          <div className="text-slate-500 text-xs">25 due this week</div>
        </div>
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
          {segmentData.map((s) => (
            <div key={s.name} className="rounded-xl border p-3 bg-white">
